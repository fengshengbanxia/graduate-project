...
window.onload = function(){
    ...
    // 监听a标签的点击，根据_href切换到iframe的src
    $('.left-sidebar .sidebar-nav a').click(function(){
        let  _href = $(this).attr("_href");
        let iframe = $(".a-iframe")
        if(iframe && _href.includes(".html")) iframe.attr('src',_href)
    })
}
...
function autoLeftNav() {
    if ($(window).width() < 1024) {
        console.log("已经变化了",$('.left-sidebar')[0])
        $('.left-sidebar').addClass('active');
    } else {
        $('.left-sidebar').removeClass('active');
    }
